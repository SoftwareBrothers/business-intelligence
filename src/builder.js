...
class Builder {
  ...
  async init() {
    ...
    await Promise.all(this.members.map(async (member, i) => {
      let hoursAvailable = await this.countWorkingHours(member)
      let hours = {
        hoursAvailable,
  ...
  async fetchPrices(){
    ...
    let prices = await this.sheet.values({range: `${projectSheet}!A${offset}:C${offset+100}`})
  ...
  async countWorkingHours(member){
    let schedules = await this.tempo.schedule({
    ...
    return schedules.reduce((m, day) => {
      if (day.type === 'WORKING_DAY') {
        m = m + day.requiredSeconds/inHour
      }
      return m
    }, 0)
  }
  ...
  async countWorkedHours(member){
    let worklogs = await this.tempo.worklogs({
      username: member.username,
      from: moment(this.month).format('YYYY-MM-DD'),
      to: moment(this.month).endOf('month').format('YYYY-MM-DD'),
    })

    let hours = worklogs.reduce((m, worklog) => {
      let h = worklog.billableSeconds/inHour
      // changes 'ST-172' to 'ST'
      let issueKey = worklog.issue.key
      ...
      if (this.prices[projectKey].price.value > 0) {
        let ammountFormula = `${m.billableAmount} + ${this.prices[projectKey].priceCell}*${h}`
  ...
  async countPlannedHours(member){
    let plans = await this.tempo.plans({
      username: member.username,
      from: moment(this.month).format('YYYY-MM-DD'),
      to: moment(this.month).endOf('month').format('YYYY-MM-DD'),
    })

    let hours = plans.reduce((m, plan) => {
      let h = plan.dates.values[0].timePlannedSeconds/inHour

      if (plan.planItem.type === 'PROJECT') {
        let projectKey = plan.planItem.self.split('/').pop()
        if (this.prices[projectKey].price.value > 0) {
          let ammountFormula = `${m.billableAmount} + ${this.prices[projectKey].priceCell}*${h}`
          m.billableAmount = ammountFormula
          m.hoursBillable += h
